...
def indexing(index_name, pmcidListFile):
    ...
    for pmid in pmcidlist:
        ...
        res = es.index(index=index_name, doc_type='article', id=pmid, body=input_D, timeout=3600, request_timeout=100000)

for topicNum in range(11, 21):
    index_name = 'dfr_2014_topic{0}'.format(topicNum)
    pmcidListFile = '2014_mixed_topic_keyword/topic{0}.csv'.format(topicNum)
