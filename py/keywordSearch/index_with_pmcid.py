...
es = Elasticsearch([{'host':'210.107.192.20', 'port':9200}])
...
def indexing(index_name, pmcidListFile):
    ...
    for pmid in pmcidlist:
        ...
        res = es.get(index='dfr_total', doc_type='article', id=pmid)
        ...
        res = es.index(index=index_name, doc_type='article', id=pmcid, body=input_D, timeout=3600, request_timeout=100000)

for topicNum in range(1, 31):
    index_name = 'dfr_ngram_2015_desc_topic{0}'.format(topicNum)
    pmcidListFile = '2015_desc_topic_keyword/topic{0}.csv'.format(topicNum)
